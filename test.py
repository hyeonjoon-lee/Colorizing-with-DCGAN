...
for epoch in range(1, EPOCH+1):
    ...
    for batch_idx, (sample, target) in enumerate(train_loader):
        ...
        if USE_GLOBAL:
            ...
            gen_total_loss += (1 / 300) * loss_class
        ...
        if batch_idx % 300 == 0:
            with torch.no_grad():
                if USE_GLOBAL:
                    to_print = f"Epoch [{epoch}/{EPOCH}] Batch {batch_idx}/{len(train_loader)} " \
                               f"Loss Disc:{disc_total_loss:.4f} (real:{loss_real}/fake:{loss_fake}), " \
                               f"Loss Gen:{gen_total_loss:.4f} (adversarial:{loss_adversarial}/l1:{loss_l1}/class:{loss_class}) "
                else:
                    to_print = f"Epoch [{epoch}/{EPOCH}] Batch {batch_idx}/{len(train_loader)} " \
                               f"Loss Disc:{disc_total_loss:.4f} (real:{loss_real}/fake:{loss_fake}), " \
                               f"Loss Gen:{gen_total_loss:.4f} (adversarial:{loss_adversarial}/l1:{loss_l1}) "
                print(to_print)
